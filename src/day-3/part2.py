...
def main():
    ...
    asterisks = [(rect, [num for num in numbers if rect.touches(num)]) for rect in rects if rect.content == '*']
    
    gear_ratio_sum = sum(reduce(mul, (num.content for num in nums), 1) for (_, nums) in asterisks if len(nums) >= 2)
